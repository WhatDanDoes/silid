...
module.exports = function(expected, done) {
  ...
  require('../setupKeystore').then(singleton => {
    ...
    stubOauthToken([apiScope.read.roles, apiScope.update.users], (err, oauthScopes) => {
      ...
      const {accessToken, oauthTokenScope: userAssignRolesOauthTokenScope} = oauthScopes;

      /**
       * POST `/api/v2/users/:id/roles/`
       */
      const userAssignRolesScope = nock(`https://${process.env.AUTH0_M2M_DOMAIN}`, { reqheaders: { authorization: `Bearer ${accessToken}`} })
